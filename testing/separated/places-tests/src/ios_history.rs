...
fn test_import_empty() -> Result<()> {
    ...
    places::import::import_ios_history(&places_api, ios_path)?;
...
fn test_import_basic() -> Result<()> {
    ...
    let visit = IOSVisit {
        ...
        // milliseconds
        date: first_visit_ts.clone().as_millis_i64() * 1000,
        type_: 1,
        ..Default::default()
    };
    ...
    let other_visit = IOSVisit {
        ...
        site_id: 1,
        // Dates in iOS are represented as μs
        date: second_visit_ts.clone().as_millis_i64() * 1000,
        type_: 1,
        ..Default::default()
    };

    let history_table = HistoryTable(vec![history_entry]);
    let visit_table = VisitTable(vec![visit.clone(), other_visit.clone()]);
